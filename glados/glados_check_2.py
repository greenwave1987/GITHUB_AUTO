...
def generate_trend_chart(points_data, email):
    ...
    plt.title(f"Points Trend: {email}")
        ...
        for email in EMAILS:
            ...
            try:
                ...
                if check_session_by_points(page):
                    ...
                    left_days = int(
                        float(
                            status_json['data'].get('leftDays',0)
                        )
                    )

                    points_json = page.evaluate(
                        f'''
                        async () => {{
                            const r = await fetch("{POINTS_API}");
                            return await r.json();
